...
addBootScrEntry() {
    ...
    echo "fatload usb 0:1 0x9000000004000000 /nixos/$(basename $kernel)"
    echo "fatload usb 0:1 0x9000000006000000 /nixos/$(basename $initrd)"
    echo "setenv bootargs init=$path/init $extraParams"
    echo "bootm 0x9000000004000000 0x9000000006000000"
}

tmpFile="$target/extlinux/extlinux.conf.tmp.$$"

scriptFile="$target/script.txt"
...
addBootScrEntry $default default >> $scriptFile
...
mkimage -A loongarch -O linux -T script -C none -a 0 -e 0 -d $scriptFile $target/bootDefaultNixOS.scr
rm $scriptFile 
